...
| 2026-06-09 | 合并 upstream v2.1.4：Figma Bridge 自动启动（tools/figma-bridge/ + backend/src/utils/figmaBridge.js）+ COS/OSS 连通性签名检查增强 + 画布教程顶栏入口（9弹B站/Youtube）+ 放置栏刷新回填 + Prompt/@ 媒体引用修复（useUpstreamMaterials 重构）+ LLM 默认模型改 gemini-3.5-flash + Electron _post_build Figma Bridge 完整性检查。版本号保持 v2.3.0，type-check / build 通过 |
