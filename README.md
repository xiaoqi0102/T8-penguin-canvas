...
产物：`dist_electron/T8-PenguinCanvas-Setup-<version>.exe`（v1.6.2 = v1.6.1（合并上游 v1.5.8 主题图标 / 跨平台路径 + v1.5.9 EVA 主题 + v1.6.0 EVA 浅色 + fork 七牛/grsai 全功能）+ 修复七牛 gemini-3.1-flash-image-preview 比例参数不生效（按 model 分流：gemini 走 image_config 嵌套协议，openai/gpt-image-2 维持 OpenAI 兼容 size/quality），安装包约 110 MB）
